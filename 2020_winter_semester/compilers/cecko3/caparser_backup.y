...
%type<cecko::CKTypeObs> type_specifier type_name
%type<casem::DeclarationSpecifier> declaration_specifier
%type<std::vector<casem::DeclarationSpecifier>> declaration_specifiers
%type<std::vector<casem::Declarator>> init_declarator_list
%type<DeclarationSpecifier> type_specifier_qualifier
%type<Declarator> declarator init_declarator direct_declarator
...
declaration_body: declaration_specifiers init_declarator_list_opt

declaration_specifiers: declaration_specifier {
                            auto decl = std::vector< DeclarationSpecifier>();
                            decl.push_back($1);
                            $$ = decl;
                        }
...
declaration_specifier: TYPEDEF { $$ = DeclarationSpecifier(true, false); }
                     | type_specifier_qualifier { $$ = $1; }
                     ;

init_declarator_list: init_declarator {
                        auto vec = std::vector< Declarator>;
                        vec.push_back($1);
                        $$ = vec;
                    }
...
type_specifier: VOID { $$ = ctx->get_void_type(); }
              | ETYPE { $$ = get_etype($1, ctx); }
              | struct_or_union_specifier { $$ = ctx->get_void_type(); }
              | enum_specifier { $$ = ctx->get_void_type(); }
              | type_name { $$ = $1; }
              ;

struct_definition_header: STRUCT IDF
                        ;

struct_or_union_specifier: struct_definition_header LCUR member_declaration_list RCUR
                         | STRUCT IDF
                         ;
...
type_specifier_qualifier: type_specifier { $$ = DeclarationSpecifier($1); }
                        | CONST { $$ = DeclarationSpecifier(false, true); }
                        ;
...
direct_declarator: IDF { $$ = Declarator($1); }
...
type_name: TYPEIDF { $$ = ctx->find_typedef($1)->get_type_pack.type(); }
            ;
